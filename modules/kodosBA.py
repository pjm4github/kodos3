...
"""
Module implementing KodosBA.
"""
from PyQt5.QtWidgets import QMainWindow,  QAction
from PyQt5.QtGui import QPixmap,  QIcon
from PyQt5.QtCore import pyqtSignal

from modules.Ui_kodosBA import Ui_MainWindow
...
image16_data = \
    ...
    "\x82"
class KodosBA(QMainWindow, Ui_MainWindow):
    """
    ...
    def __init__(self, parent = None):
        ...
        self.fileOpenAction = QAction("fileOpenAction", self)
        self.fileOpenAction.setIcon(QIcon(self.image3))
        self.fileSaveAction = QAction("fileSaveAction", self)
        self.fileSaveAction.setIcon(QIcon(self.image4))
        self.fileSaveAsAction = QAction("fileSaveAsAction",self)
        self.fileExitAction = QAction("fileExitAction",self)
        self.editUndoAction = QAction("editUndoAction",self)
        self.editUndoAction.setIcon(QIcon(self.image5))
        self.editRedoAction = QAction("editRedoAction",self)
        self.editRedoAction.setIcon(QIcon(self.image6))
        self.editCutAction = QAction("editCutAction",self)
        self.editCutAction.setIcon(QIcon(self.image7))
        self.editCopyAction = QAction("editCopyAction",self)
        self.editCopyAction.setIcon(QIcon(self.image8))
        self.editPasteAction = QAction("editPasteAction",self)
        self.editPasteAction.setIcon(QIcon(self.image9))
        self.examineAction = QAction("examineAction",self)
        self.examineAction.setCheckable(1)
        self.examineAction.setIcon(QIcon(self.image10))
        self.helpContentsAction = QAction("helpContentsAction",self)
        self.helpIndexAction = QAction("helpIndexAction",self)
        self.helpAboutAction = QAction("helpAboutAction",self)
        self.regexChangedAction = QAction("regexChangedAction",self)
        self.editPauseAction = QAction("editPauseAction",self)
        self.editPauseAction.setCheckable(1)
        self.editPauseAction.setIcon(QIcon(self.image11))
        self.editPreferencesAction = QAction("editPreferencesAction",self)
        self.helpHelpAction = QAction("helpHelpAction",self)
        self.helpHelpAction.setIcon(QIcon(self.image12))
        self.helpPythonHelpAction = QAction("helpPythonHelpAction",self)
        self.helpRegexReferenceAction = QAction("helpRegexReferenceAction",self)
        self.helpRegexReferenceAction.setIcon(QIcon(self.image13))
        self.helpVisitWebsiteAction = QAction("helpVisitWebsiteAction",self)
        self.helpVisitWebsiteAction.setIcon(QIcon(self.image14))
        self.helpCheckForUpdateAction = QAction("helpCheckForUpdateAction",self)
        self.helpReportBugAction = QAction("helpReportBugAction",self)
        self.noopAction = QAction("noopAction",self)
        self.noopAction.setEnabled(1)
        self.noopAction.setIcon(QIcon(self.image15))
        self.fileImportFileAction = QAction("fileImportFileAction",self)
        self.fileImportURLAction = QAction("fileImportURLAction",self)
        self.helpRegexLibAction = QAction("helpRegexLibAction",self)
        self.helpRegexLibAction.setIcon(QIcon(self.image16))
        self.FileRevertAction = QAction("FileRevertAction",self)
        ...
        self.connect(self.dotallCheckBox,str("toggled(bool)"),self.checkbox_slot)
        self.connect(self.editCopyAction,str("activated()"),self.editCopy)
        self.connect(self.editCutAction,str("activated()"),self.editCut)
        self.connect(self.editPasteAction,str("activated()"),self.editPaste)
        self.connect(self.editRedoAction,str("activated()"),self.editRedo)
        self.connect(self.editUndoAction,str("activated()"),self.editUndo)
        self.connect(self.examineAction,str("activated()"),self.examine)
        self.connect(self.fileExitAction,str("activated()"),self.close)
        self.connect(self.fileNewAction,str("activated()"),self.fileNew)
        self.connect(self.fileOpenAction,str("activated()"),self.fileOpen)
        self.connect(self.fileSaveAction,str("activated()"),self.fileSave)
        self.connect(self.fileSaveAsAction,str("activated()"),self.fileSaveAs)
        self.connect(self.helpAboutAction,str("activated()"),self.helpAbout)
        self.connect(self.helpContentsAction,str("activated()"),self.helpContents)
        self.connect(self.helpHelpAction,str("activated()"),self.helpHelp)
        self.connect(self.helpIndexAction,str("activated()"),self.helpIndex)
        self.connect(self.ignorecaseCheckBox,str("toggled(bool)"),self.checkbox_slot)
        self.connect(self.localeCheckBox,str("toggled(bool)"),self.checkbox_slot)
        self.connect(self.matchNumberSpinBox,str("valueChanged(int)"),self.match_num_slot)
        self.connect(self.multilineCheckBox,str("toggled(bool)"),self.checkbox_slot)
        self.connect(self.regexMultiLineEdit,str("textChanged()"),self.regex_changed_slot)
        self.connect(self.stringMultiLineEdit,str("textChanged()"),self.string_changed_slot)
        self.connect(self.unicodeCheckBox,str("toggled(bool)"),self.checkbox_slot)
        self.connect(self.verboseCheckBox,str("toggled(bool)"),self.checkbox_slot)
        self.connect(self.editPauseAction,str("toggled(bool)"),self.pause)
        self.connect(self.editPreferencesAction,str("activated()"),self.preferences)
        self.connect(self.helpReportBugAction,str("activated()"),self.report_bug)
        self.connect(self.helpPythonHelpAction,str("activated()"),self.helpPythonRegex)
        self.connect(self.helpRegexReferenceAction,str("activated()"),self.reference_guide)
        self.connect(self.helpVisitWebsiteAction,str("activated()"),self.kodos_website)
        self.connect(self.helpCheckForUpdateAction,str("activated()"),self.check_for_update)
        self.connect(self.replaceTextEdit,str("textChanged()"),self.replace_changed_slot)
        self.connect(self.replaceNumberSpinBox,str("valueChanged(int)"),self.replace_num_slot)
        self.connect(self.fileImportFileAction,str("activated()"),self.importFile)
        self.connect(self.fileImportURLAction,str("activated()"),self.importURL)
        self.connect(self.helpRegexLibAction,str("activated()"),self.helpRegexLib)
        self.connect(self.regexMultiLineEdit,str("textChanged()"),self.kodos_edited_slot)
        self.connect(self.stringMultiLineEdit,str("textChanged()"),self.kodos_edited_slot)
        self.connect(self.ignorecaseCheckBox,str("toggled(bool)"),self.kodos_edited_slot)
        self.connect(self.replaceTextEdit,str("textChanged()"),self.kodos_edited_slot)
        self.connect(self.multilineCheckBox,str("toggled(bool)"),self.kodos_edited_slot)
        self.connect(self.dotallCheckBox,str("toggled(bool)"),self.kodos_edited_slot)
        self.connect(self.verboseCheckBox,str("toggled(bool)"),self.kodos_edited_slot)
        self.connect(self.localeCheckBox,str("toggled(bool)"),self.kodos_edited_slot)
        self.connect(self.unicodeCheckBox,str("toggled(bool)"),self.kodos_edited_slot)
        self.connect(self.FileRevertAction,str("activated()"),self.revert_file_slot)
    ...
    @pyqtSignal()
    def on_fileImportFileAction_triggered(self, url, html):
        """
        Slot documentation goes here.
        """
        self.url = url
    ...
    @pyqtSignal("")
    def on_fileImportURLAction_triggered(self):
        """
        Slot documentation goes here.
        """
        self.urldialog = URLDialog(self, self.url)
